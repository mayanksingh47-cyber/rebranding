...
class UnifiedPDFRebrander:
    ...
    def _replace_purple_colors_in_image(self, img: Image.Image) -> Image.Image:
        """
        Replace purple colors with orange/melon (#FF7E51) in image.
    ...
    def rebrand_pdf(self, source_file: bytes, source_filename: str) -> Tuple[bytes, str]:
        ...
        try:
            ...
            for img in page_images:
                img = self._replace_purple_colors_in_image(img)
...
class UnifiedWordRebrander:
    ...
    def _replace_run_purple_color(self, run):
        ...
        try:
            ...
            if run.font.color and run.font.color.rgb:
                ...
                if self._is_purple_color(color_hex):
                    run.font.color.rgb = self.DXC_MELON
                    return True
    ...
    def _replace_purple_in_numbering(self, doc: Document):
        """
    ...
    def _replace_purple_in_styles(self, doc: Document):
        """
                ...
                try:
                    if hasattr(style, 'font') and style.font and style.font.color:
                        if style.font.color.rgb:
                            ...
                            if self._is_purple_color(color_hex):
                                style.font.color.rgb = self.DXC_MELON
                except:
                    pass
                ...
                try:
                    for para in doc.paragraphs:
                        if para.style.name == style_name:
                            # This paragraph uses the purple style - apply orange to ALL runs
    ...
    def _replace_purple_backgrounds(self, doc: Document):
        """
        Replace purple background/shading colors with MELON.
        ...
        self._replace_purple_shape_fills(doc)
    
    def _replace_purple_shape_fills(self, doc: Document):
        """
                    ...
                    if tag_name == 'schemeclr':
                        ...
                        if any(x in val.lower() for x in ['accent', 'dk1', 'dk2', 'tx1', 'tx2']):
                            ...
                            if parent is not None:
                                # Replace schemeClr with srgbClr MELON
                                parent.remove(elem)
                                new_srgb = OxmlElement('a:srgbClr')
                                new_srgb.set('val', melon_rgb)
                ...
                try:
                    ...
                    if scheme_clr is not None:
                        ...
                        if any(x in val.lower() for x in ['accent', 'dk1', 'dk2', 'tx1', 'tx2']) or 'purple' in val.lower():
                            # This is a theme color - replace entirely with MELON
            ...
            for spPr in list(root_element.iter('{%s}spPr' % a_ns_full)):
                try:
                    ...
                    if solid_fill is not None:
                        ...
                        # If purple, replace with MELON
                        if is_purple:
                            # Clear and recreate with MELON
    ...
    def _replace_purple_in_raw_xml(self, doc: Document):
        ...
        def replace_in_xml_element(element):
            """Replace purple colors and theme colors in element's XML."""
            try:
                xml_str = etree.tostring(element, encoding='unicode')
                original_str = xml_str
                
                # Replace each known purple code (case variations)
                for purple in purple_codes:
                    for p in [purple, purple.upper(), purple.lower()]:
                        xml_str = xml_str.replace(f'"{p}"', f'"{melon}"')
                        xml_str = xml_str.replace(f'"#{p}"', f'"#{melon}"')
                        xml_str = xml_str.replace(f'val="{p}"', f'val="{melon}"')
                
                # CRITICAL: Replace theme color references with direct MELON color
                # Pattern: <a:schemeClr val="accent..."/> -> <a:srgbClr val="FF7E51"/>
                theme_pattern = r'<a:schemeClr[^>]*val="accent[^"]*"[^/]*/>'
                xml_str = re.sub(theme_pattern, f'<a:srgbClr val="{melon}"/>', xml_str)
                
                # Also handle schemeClr with child elements
                theme_pattern2 = r'<a:schemeClr[^>]*val="accent[^"]*"[^>]*>.*?</a:schemeClr>'
                xml_str = re.sub(theme_pattern2, f'<a:srgbClr val="{melon}"/>', xml_str, flags=re.DOTALL)
                
                # Replace dk1, dk2, tx1, tx2 theme colors too
                for theme in ['dk1', 'dk2', 'tx1', 'tx2', 'lt1', 'lt2']:
                    xml_str = re.sub(
                        rf'<a:schemeClr[^>]*val="{theme}"[^/]*/>', 
                        f'<a:srgbClr val="{melon}"/>', 
                        xml_str
                    )
                
                if xml_str != original_str:
                    new_elem = etree.fromstring(xml_str.encode('utf-8'))
                    element.clear()
                    element.tag = new_elem.tag
                    element.attrib.update(new_elem.attrib)
                    element.text = new_elem.text
                    element.tail = new_elem.tail
        ...
        try:
            replace_in_xml_element(doc.element.body)
        except:
            pass
    ...
    def _replace_purple_text_colors(self, doc: Document):
        """
        Find and REPLACE purple text colors with MELON color (#FF7E51).
        ...
        self._replace_purple_in_numbering(doc)
        
        # Replace purple in document styles (all heading/paragraph styles)
        self._replace_purple_in_styles(doc)
        ...
        self._replace_purple_backgrounds(doc)
                        ...
                        if para_style.font.color.rgb:
                            ...
                            if len(clean_hex) >= 6:
                                ...
                                if self._is_purple_color(clean_hex):
                                    # Paragraph style is purple - apply orange to ALL runs
                                    for run in para.runs:
                                        run.font.color.rgb = self.DXC_MELON
                        elif para_style.font.color.theme_color:
                            theme = str(para_style.font.color.theme_color)
                            if 'ACCENT' in theme or 'PURPLE' in theme.upper():
                                # Paragraph style has purple theme - apply orange to ALL runs
                                for run in para.runs:
                                    run.font.color.rgb = self.DXC_MELON
                except:
                    pass
    ...
    def rebrand_document(self, source_file: bytes, source_filename: str) -> Tuple[bytes, str]:
        """
        Rebrand a Word document with DXC branding.
        
        - REPLACES old DXC logos anywhere in document with new logo
        - REPLACES purple text colors with MELON color (#FF7E51)
        - REPLACES header with new DXC header
        - REPLACES footer with new DXC footer
        ...
        try:
            ...
            # This converts purple shapes, text, tables to orange
            self._replace_purple_text_colors(doc)
    ...
    if uploaded_files:
        ...
        if process_btn:
            ...
            with st.spinner("Processing documents..."):
                results = st.session_state.unified_rebrander.process_files(uploaded_files)
