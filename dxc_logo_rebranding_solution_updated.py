...
# CEO-level metrics persistence (documents uploaded / converted)
METRICS_FILE = Path(__file__).parent / "rebranding_metrics.json"


def load_metrics() -> Dict[str, int]:
    """Load persisted document upload/conversion counts from JSON."""
    try:
        if METRICS_FILE.exists():
            with open(METRICS_FILE, "r", encoding="utf-8") as f:
                data = json.load(f)
                return {
                    "documents_uploaded": int(data.get("documents_uploaded", 0)),
                    "documents_converted": int(data.get("documents_converted", 0)),
                }
    except (json.JSONDecodeError, IOError):
        pass
    return {"documents_uploaded": 0, "documents_converted": 0}


def save_metrics(documents_uploaded: int, documents_converted: int) -> None:
    """Persist document upload/conversion counts to JSON."""
    try:
        with open(METRICS_FILE, "w", encoding="utf-8") as f:
            json.dump(
                {"documents_uploaded": documents_uploaded, "documents_converted": documents_converted},
                f,
                indent=2,
            )
    except IOError:
...
def load_css():
    ...
    /* CEO-level metrics dashboard - professional KPI cards */
    .ceo-metrics-wrapper {
    ...
    .ceo-metric-card {
        ...
        display: flex;
        align-items: stretch;
        box-shadow: 0 2px 8px rgba(14, 16, 32, 0.08);
        border: 1px solid rgba(0,0,0,0.06);
    }
    .ceo-metric-card.orange {
        background: linear-gradient(135deg, #FFF8F0 0%, #FFEDD5 50%, #FEF3C7 100%);
        background-image: url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 0L60 30L30 60L0 30z' fill='%23EF8900' fill-opacity='0.04'/%3E%3Cpath d='M0 0L30 30L0 60L-30 30z' fill='%23EF8900' fill-opacity='0.06'/%3E%3C/svg%3E");
    }
    .ceo-metric-card.green {
        background: linear-gradient(135deg, #F0FDF4 0%, #DCFCE7 50%, #BBF7D0 100%);
        background-image: url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 0L60 30L30 60L0 30z' fill='%23198754' fill-opacity='0.06'/%3E%3Cpath d='M0 0L30 30L0 60L-30 30z' fill='%23198754' fill-opacity='0.08'/%3E%3C/svg%3E");
    }
    .ceo-metric-accent {
        width: 6px;
        flex-shrink: 0;
    }
    .ceo-metric-card.orange .ceo-metric-accent {
        background: linear-gradient(180deg, #EF8900, #E07A00);
    }
    .ceo-metric-card.green .ceo-metric-accent {
    ...
    .ceo-metric-card.orange .ceo-metric-value { color: #B45309; }
    .ceo-metric-card.green .ceo-metric-value { color: #0D6E3F; }
    .ceo-metric-unit {
        font-size: 1rem;
        font-weight: 700;
        margin-top: 0.15rem;
    }
    .ceo-metric-card.orange .ceo-metric-unit { color: #B45309; }
    .ceo-metric-card.green .ceo-metric-unit { color: #0D6E3F; }
    .ceo-metric-label {
        font-size: 0.85rem;
        font-weight: 500;
        margin-top: 0.25rem;
    ...
    .ceo-metric-card.orange .ceo-metric-label { color: #92400E; }
    .ceo-metric-card.green .ceo-metric-label { color: #0D6E3F; }
    .ceo-metric-plus {
        position: absolute;
        top: 1rem;
        right: 1rem;
    ...
    .ceo-metric-card.orange .ceo-metric-plus { color: #B45309; }
    .ceo-metric-card.green .ceo-metric-plus { color: #0D6E3F; }
...
def main():
    ...
    <div class="enterprise-header">
        <div class="header-left">
            ...
            <div class="header-content">
                <div class="header-app-name">Reface</div>
                <div class="enterprise-title">An AI Accelerator for Enterprise Rebranding</div>
                <div class="enterprise-subtitle">One Upload. One Brand. Zero Effort.</div>
            </div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    # CEO-level metrics: load from file if not in session (persisted across runs)
    if 'documents_uploaded' not in st.session_state or 'documents_converted' not in st.session_state:
        metrics = load_metrics()
        st.session_state.documents_uploaded = metrics["documents_uploaded"]
        st.session_state.documents_converted = metrics["documents_converted"]
    
    # CEO-level metrics dashboard (documents uploaded / documents converted)
    up = st.session_state.documents_uploaded
    conv = st.session_state.documents_converted
    st.markdown(f"""
    <div class="ceo-metrics-wrapper">
        <div class="ceo-metrics-header">DOCUMENT REBRANDING METRICS</div>
        <div class="ceo-metrics-row">
            <div class="ceo-metric-card orange">
                <div class="ceo-metric-accent"></div>
                <div class="ceo-metric-content">
                    <span class="ceo-metric-plus">+</span>
                    <span class="ceo-metric-value">{up:,}</span>
                    <span class="ceo-metric-unit">documents</span>
                    <span class="ceo-metric-label">documents uploaded</span>
                </div>
            </div>
            <div class="ceo-metric-card green">
                <div class="ceo-metric-accent"></div>
                <div class="ceo-metric-content">
                    <span class="ceo-metric-plus">+</span>
                    <span class="ceo-metric-value">{conv:,}</span>
                    <span class="ceo-metric-unit">documents</span>
                    <span class="ceo-metric-label">documents converted</span>
                </div>
            </div>
        </div>
    </div>
    ...
    if uploaded_files:
        ...
        if process_btn:
            ...
            with st.spinner("Processing documents..."):
                ...
                # Update and persist CEO-level metrics
                st.session_state.documents_uploaded += len(uploaded_files)
                st.session_state.documents_converted += results["stats"]["success_count"]
                save_metrics(st.session_state.documents_uploaded, st.session_state.documents_converted)
    ...
    <div class="enterprise-footer">
        <div class="footer-top">
            <div class="footer-credits">
                ...
                <span>Product Owner <strong>Kirti Var Bisht & Jasveen Kaur</strong></span>
                <span class="footer-separator">|</span>
        ...
        <div class="footer-bottom">
            <div class="footer-copyright">DXC TECHNOLOGY &copy; 2026</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
